...
import type { Analysis, CreateAnalysis } from '@/types/analysis'
  ...
  <div class="flex flex-col px-60 h-svh justify-start gap-8 mt-10 ">
    ...
    <div class="flex flex-col gap-3">
      <div v-if="analysisToCreate.resume == null" class="w-full h-56 flex flex-col justify-center items-center border-dashed border-2 border-[#CFDBE8] rounded-lg " ref="dropZoneRef">
        ...
        <input type="file" class="text-[#0D141C]" placeholder="Or click to browse" @change="e => { if (e.target.files && e.target.files.length > 0) analysisToCreate.resume = e.target.files[0] }">
      </div>
      <div v-else class="w-full h-56 flex flex-col justify-center items-center border-dashed border-2 border-[#cfe8d3] rounded-lg " ref="dropZoneRef">
      ...
      <div class="flex flex-col gap-3">
      <div class="w-full h-56 flex flex-col justify-center items-center border-dashed border-2 border-[#CFDBE8] rounded-lg " ref="dropZoneRef">
        <h2 class="text-[#0D141C] text-xl font-bold">Job description here (In Progress...)</h2>
        <input type="text" class="text-[#0D141C] w-full h-full" placeholder="click"  v-model="analysisToCreate.description">
      </div>
